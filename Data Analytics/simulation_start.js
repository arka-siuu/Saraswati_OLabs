function startActivity(moduleId) {
    ...
    console.log(`Starting activity: UserID=${userId}, Module=${moduleId}, StartTime1=${startTime1}`);

    fetch("http://localhost:5000/start_activity_simulation", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId, moduleId, startTime1 })
    })
    .then(res => res.json())
    .then(data => {
